module.exports = function( grunt )
{
	...
	grunt.registerTask( 'create-block-component', 'Create a block component (template, viewmodel and controller).', function( n )
	{
		...
		var componentScaffoldTargets = [
			...
			},
			{
				...
				componentIdSubFolder : true,
				filename: function(componentId) {
					return dashesToCapitalized(componentId) + 'TransitionController';
				}
			},
			{
				template: 'options',
		...
		// generate a list of all filenamesc
		var filenames = {};
		...
		grunt.log.subhead( 'Block-component creation succesful!' );
		grunt.log.writeln( 'To use your block-component, add it to the blockConfig.ts and the blocks.ts' );
	} );
	...
	function scaffoldStyleCollectionFile(componentId, subfolder) {
		...
		var filename = grunt.config( 'create-block-component.options.styleCollectionFilename' );
	...
	function showHelp()
	{
		grunt.log.subhead( 'Usage: grunt create-block-component --name=[COMPONENT-NAME]' );
	...
	function getConfigDir( name ) {
		return grunt.config( 'create-block-component.options.directories.' + name );
	}
