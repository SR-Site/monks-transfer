module.exports = function(grunt)
{
	...
	grunt.registerMultiTask('version', 'Apply versioning to the build folder', function()
	{
		...
		var targetIndex = grunt.config.data.buildDir + 'index.php';
		var done = this.async();
		...
		fs.move(
			...
			{
				// Remove old folders
				deleteFolderRecursive(grunt.config.data.buildDir + 'deploy/');
				...
				fs.move(
					grunt.config.data.buildDir + '/version',
					...
					function()
					{
						...
						fs.move(
							grunt.config.data.buildDir + '/temp',
							grunt.config.data.buildDir + '/version/' + deployDate,
							function()
							{
								done();
							}
