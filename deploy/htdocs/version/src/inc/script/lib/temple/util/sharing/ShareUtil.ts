 ...
 *   ```typescript
 *   public shareUtil:SharingUtil;
 *   public twitterSharer:ShareMethodTwitter;
 *   ...
 *   this.shareUtil = new SharingUtil();
 *   this.twitterSharer = new ShareMethodTwitter();
 *   this.shareUtil.addMethod(this.twitterSharer);
 *   this.shareUtil.addMethod(new FacebookSharerShareMethod());
 *   ```
 ...
 *    ```html
 *    <ul data-bind="foreach : shareUtil.methods">
...
export default class ShareUtil extends Destructible {
	 ...
	 *
	 * @property defaultBindingOptions
	 * @type string
	 * @static
	 */
	 ...
	 *
	 * @property methods
	 * @type KnockoutObservableArray<IShareMethod>
	 */
	 ...
	 * @method clickBindingHandler
	 * @param koData {any} The knockout data that is passed with this function call
	 * @param event {MouseEvent} The mouse event that triggered the handler
	...
	{
		var id:number = ko.utils.domData.get(<Element> event.currentTarget,
	...
	{
		var shareUtilInstance:ShareUtil = ko.unwrap(ko.unwrap(valueAccessor()).shareUtil);
		if(!shareUtilInstance)
		{
			if(ShareUtil._instances[0])
			{
				shareUtilInstance = ShareUtil._instances[0];
			}
			else
			{
		...
		var id = shareUtilInstance._idCounter++;
		ko.utils.domData.set(element, ShareUtil._ELEMENT_ID_DOMDATA_KEY, id);

		ko.bindingHandlers.click.init(element, function()
		{
			return shareUtilInstance.clickBindingHandler
	                                   ...
	                                   viewModel:any, bindingContext:KnockoutBindingContext):void
	{
		...
		var shareUtilInstance = options.shareUtil || ShareUtil._instances[0];
		if(!shareUtilInstance)
		{
			throw new Error('ShareUtil: cannot find a valid ShareUtil instance.')
		}
		...
		shareUtilInstance._shareOptions[id] = options;
	}
	...
	public destruct():void
	{
		...
		if(index >= 0)
		{
			ShareUtil._instances.splice(index, 1);
		}
		super.destruct();
	}
}
 ...
 * 
 * @class IShareUtilKOBindingOptions
 * @namespace temple.util.sharing
 */
export interface IShareUtilKOBindingOptions extends IShareMethodOptions {
	 ...
	 *
	 * @property shareUtil
	 * @type temple.util.sharing.ShareUtil
	 * @optional
	 */
	shareUtil? : ShareUtil;
	/**
	 * A IShareMethod instance you want to use for sharing, or the string id for that
	 *
	 * @property method
	 * @type IShareMethod|string
	 */
