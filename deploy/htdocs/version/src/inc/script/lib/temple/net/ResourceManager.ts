...
class ResourceManager extends EventDispatcher
{
	 ...
	 * @returns {IManifest[]}
	 */
	public static getManifestByAssetIDList(list:Array<AssetId>):Array<IManifest>
	{
	 ...
	 * @returns {IManifest[]}
	 */
	public static getManifestBySrcList(list:Array<string>):Array<IManifest>
	{
		var manifest = [];
		for(var i = 0; i < list.length; i++)
		{
	 ...
	 * @return void
	 */
	public load(assetList:Array<any>, onProcess:(event:createjs.Event) => void = null, onComplete:(event:createjs.Event) => void = null):void
	{
		...
		var manifest:Array<IManifest>;

		switch(type)
		{
			case 'number':
			{
				manifest = ResourceManager.getManifestByAssetIDList(<Array<AssetId>> assetList);
				break;
			}
			case 'string':
			{
				manifest = ResourceManager.getManifestBySrcList(<Array<string>> assetList );
				break;
			}
		}
	 ...
	 */
	public loadImagesAsync(assetList:Array<any>, onProgress:(percent:number) => any = null, list:{[id:string]:HTMLImageElement} = {}):void
	{
		var type = typeof assetList[0];
		var manifest:Array<IManifest>;
	...
	{
		this.dispatchEvent(new BasicEvent('complete'));
	}
	...
	public getByImageBySrc(src:string, onLoad:(e:DataEvent<HTMLImageElement>) => any):any
	{
		...
		{
			this.addEventListener(eventId, onLoad, null).once();
		...
		else
		{
			...
			if(!hasContent)
			{
				this.addEventListener(eventId, onLoad).once();
			}
			else
			{
