...
class StringUtils
{
	...
	public static replaceVars(value:string, object:any, keepIrreplaceableVars:boolean = true, debug:boolean = false):string
	{
		...
		return value.replace(/\$?\{([@#$%&\w\.]*)(\((.*?)\))?\}/gi, (match:string, name:string, method:boolean, args:string, offset:number, full:string) =>
		{
			var props:Array<string> = name.split(".");
			var prop:string = props.shift();
			var o:any = object;
			...
			if (o != null && prop in o)
			{
				if(typeof o[prop] == 'function' && method)
				{
					if(args)
					{
						var a:any[] = args.split(',');
						for(var i:number = 0; i < args.length; i++)
						{
							a[i] = StringUtils.replaceVars(args[i], object);
						}
					}

					if(debug)
					{
						return (o[prop]).apply(null, a);
					}
					else
					{
						try
						...
						{
							Log.log('Temple.util.type.StringUtils', 'Unable to replace var ' + match + ': ' + error.message);
						}
					}
				}
				else
	 ...
	 * @return {sting} The string without the word.
	 */
	public static removeWords(value:string, words:Array<string>, caseSensitive:boolean = true):string
	{
	 ...
	 * @param {Array<string>} seperators Array with the seperators to split on.
	 * @param {boolean} [reappendSeperator=false] Re-append the seperator after each part.
	 * @return {Array<string>} A single-dimension array with the parts.
	 */
	public static splitMultiSeperator(value:string, seperators:Array<string>, reappendSeperator:boolean = false):Array<string>
	{
		var ret:Array<string> = [value];
	 ...
	 * @static
	 * @param {Array<string>} values Array with the strings to seperate.
	 * @param {string} seperators The seperator to split on.
	 * @param {boolean} [reappendSeperator=false] Re-append the seperator after each part.
	 * @return {Array<string>} A single-dimension array with the parts.
	 */
	public static splitElements(values:Array<string>, seperator:string, reappendSeperator:boolean = false):Array<string>
	{
		var ret:Array<string> = [];
		for(var i:number = 0; i < values.length; i++)
		{
			var split:Array<string> = values[i].split(seperator);
	 ...
	 * @method trimAll
	 * @static
	 * @param {Array<string>} values Array with the strings to trim.
	 * @return {Array<string>} An array with the trimmed strings.
	 */
	public static trimAll(values:Array<string>):Array<string>
	{
		for(var i:number = 0; i < values.length; i++)
		{
	 ...
	 * @static
	 * @param {Array<string>} values Array with the strings to trim.
	 * @return {Array<string>} An array with the trimmed and filtered strings.
	 */
	public static trimAllFilter(values:Array<string>):Array<string>
	{
		var ret:Array<string> = [];
	 ...
	 * @static
	 * @param {Array<string>} values The string to clean.
