...
import GotoEventItem from "../event/GotoEventItem";
...
class GaiaHQ extends EventDispatcher
{
	...
	public addListener(eventName:string, target:(event:GaiaEvent) => any, hijack:boolean, onlyOnce:boolean):(removeHijack?:boolean) => void
	{
		if (this.listeners[eventName] != null)
		{
			...
			{
				listener._onHijackCompleteDelegate = <(event:IEvent) => any>this.onHijackComplete.bind(this);
	...
	{
		this.dispatchEvent(new BasicEvent(GaiaHQ.TRANSITION_OUT));
	}
	...
	{
		this.dispatchEvent(new BasicEvent(GaiaHQ.PRELOAD));
	}
	...
	{
		this.dispatchEvent(new BasicEvent(GaiaHQ.TRANSITION));
	}
	...
	{
		this.dispatchEvent(new BasicEvent(GaiaHQ.TRANSITION_IN));
	}
	...
	{
		this.dispatchEvent(new BasicEvent(GaiaHQ.COMPLETE));
	...
	private onHijackComplete(event:IEvent):void
	{
...
{
	public event:string;
	public target:(event:BasicEvent, data?:any) => any;
	public hijack:boolean;
	public onlyOnce:boolean;
	public completed:boolean;
	public dispatched:boolean;

	_onHijackCompleteDelegate:(event:IEvent) => any;
