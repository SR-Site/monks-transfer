...
import BranchLoaderEvent from "../event/BranchLoaderEvent";
...
import * as utils from "lib/temple/util/Utils";
...
class SiteController extends EventDispatcher
{
	...
	constructor(sv:SiteView)//, preloader:IPreloader)
	{
		...
		this.branchLoader.addEventListener(
			...
			<(event:IEvent) => any>this.onPreloadComplete.bind(this)
		);
				...
				if (!SiteController.isTransitioning && !SiteController.isLoading)
				{
					...
					if (event.flow == null)
					{
						...
						else
						{
							...
							var prevArray:Array<IPageAsset> = BranchTools.getPagesOfBranch(SiteController.currentBranch);
							var newArray:Array<IPageAsset> = BranchTools.getPagesOfBranch(validBranch);
							var i:number;
	...
	public onTransitionOut(event:IEvent):void
	{
	...
	public onTransitionIn(event:IEvent):void
	{
		if (!this.checkQueuedBranch())
		{
			SiteController.isTransitioning = true;
	...
	public onTransition(event:IEvent):void
	{
		if (!this.checkQueuedBranch())
		{
			SiteController.isTransitioning = true;
	...
	public onPreload(event:IEvent):void
	{
		if (!this.checkQueuedBranch())
		{
			SiteController.isLoading = true;
	...
	public onComplete(event:IEvent):void
	{
		this.checkQueuedBranch();
	}

	public onPreloadComplete(event:IEvent):void
	{
