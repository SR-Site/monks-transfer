...
class BranchTools
{
	 ...
	 * @returns {*[]}
	 */
	public static getPagesOfBranch(branch:string):Array<IPageAsset>
	{
	...
	public static getPopupBranch(popupId:string, branch:string):string
	{
		...
		var currentBranchArray:Array<string> = validBranch.split('/');

		// first remove all the current popup segments
		while (this.getPage(currentBranchArray.join('/')).type == PageType.POPUP)
		{
	...
	public static resolveBranch(branch:string, resolveFromBranch:string):string
	{
		...
		if (branch.charAt(0) == '.')
		{
			var currentPath:Array<string> = resolveFromBranch.split('/');
			var instructions:Array<string> = branch.split('/');
