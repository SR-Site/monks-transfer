...
interface DraggableVars
{
	 ...
	 */
	liveSnap?:((endValue:number)=>number|boolean|Array<number>)|{x?:(endValue:number)=>number|Array<number>, y?:(endValue:number)=>number|Array<number>}|boolean;

	/**
	 * snap : Function | Object | Array - allows you to define rules for where the element can land after it gets
	 * released. For example, maybe you want the rotation to always end at a 90-degree increment or you want the x and y
	 * values to be exactly on a grid (whichever cell is closest to the natural landing spot) or maybe you want it to
	 * land on a very specific value. You can define the snap in any of the following ways:
	 */
	snap?:((endValue:number)=>number|boolean|Array<number>)| {x?:(endValue:number)=>number|Array<number>, y?:(endValue:number)=>number|Array<number>};
