...
import BlockHeroSecondaryController from "./BlockHeroSecondaryController";
...
class BlockHeroSecondaryTransitionController extends AbstractTransitionController
{
	...
	protected setupTransitionInTimeline(): void
	{
		...
		this._transitionInTimeline.from(this.element, 0.5, { opacity: 0 });

		// Run the background switch
		this._transitionInTimeline.add(() => this._parentController.changeBackgroundImage(), 0);
		...
		this._transitionInTimeline.from(heading, 0.8, { y: 50, autoAlpha: 0, ease: Expo.easeOut }, '=+0.5');

		if( copy ) this._transitionInTimeline.from(copy, 0.8, { y: 50, autoAlpha: 0, ease: Expo.easeOut }, '=-0.7');
		if( button ) this._transitionInTimeline.from(button, 0.8, { y: 50, autoAlpha: 0, ease: Expo.easeOut }, '=-0.7');

		// Slide in the secondary triangle
		this._transitionInTimeline.from(this.element.querySelector('.secondary-background-triangle'), 1.5, {
			x: window.innerWidth,
			y: -window.innerWidth,
			ease: Expo.easeOut
		}, '=-1');
