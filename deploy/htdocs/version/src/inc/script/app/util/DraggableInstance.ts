 ...
 * @description We created this class because we were re-typing this code a lot of times. Instead of setting up a draggable instance
 * binding the on throw updates time after time again we decided on 1 instance that functions the same every time. It allows for a
 * couple of parameters that were different on the instances.
 */
class DraggableInstance extends EventDispatcher
{
	// Event names
	public static DRAG_END: string = 'drag-end';
	public static SNAP_END: string = 'snap-end';
	public static THROW_COMPLETE: string = 'throw-complete';
	public static SET_COMPLETE: string = 'set-complete';
	public static DRAG_START: string = 'drag-start';
	...
	constructor(draggableContainer: HTMLElement, options: IDraggableInstanceOptions = {})
	{
		...
		this._destructibles.add(new NativeEventListener(window, 'resize', ThrottleDebounce.debounce(this.handleResize, 100, this)));
		this._destructibles.add(new NativeEventListener(window, 'orientationchange', ThrottleDebounce.debounce(this.handleResize, 100, this)));
		...
		TweenLite.to(this._draggableElement, duration,
			{
				...
				onComplete: () =>
				{
					this.dispatch(DraggableInstance.SET_COMPLETE, {progress: this._progress})
				}
			});
	}
	...
	{
		this.dispatch(DraggableInstance.THROW_COMPLETE, {
			progress: this._progress
		});
	}
	...
	{
		// Notify the parents
		this.dispatch(DraggableInstance.DRAG_END);
	}
	...
	{
		// Notify the parents
		this.dispatch(DraggableInstance.DRAG_START);
	}
