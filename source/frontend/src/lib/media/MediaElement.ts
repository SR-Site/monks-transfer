...
import NetworkStateEnum from 'lib/media/enum/NetworkStateEnum';
import ReadyStateEnum from 'lib/media/enum/ReadyStateEnum';
...
class MediaElement {
	...
	/**
	 * A Boolean attribute; if specified, the audio will automatically begin being downloaded, even if not set to
	 * automatically play. This continues until the media cache is full, or the entire audio file has been downloaded,
	 * whichever comes first. This should only be used when it is expected that the user will choose to play the audio;
	 * for example, if the user has navigated to a page using a "Play this audio" link. This attribute was removed in
	 * Gecko 2.0 (Firefox 4 / Thunderbird 3.3 / SeaMonkey 2.1) in favor of the preload attribute.
	 *
	 * @deprecated
	 * @property autobuffer
	 */
	...
	/**
	 * The length of the media in seconds, or zero if no media data is available.  If the media data is available but
	 * the length is unknown, this value is NaN.  If the media is streamed and has no predefined length, the value is
	 * Inf.
	 *
	 * @method getDuration
	 * @return number
	 ...
	 * than normal, higher values make it play faster. The value 0.0 is invalid and throws a NOT_SUPPORTED_ERR
	 * exception.
	 *
	...
	/**
	 * If the playbackRate is negative, the media is played backwards. The audio is muted when the media plays
	 * backwards
	 * or if the fast forward or slow motion is outside a useful range (E.g. Gecko mute the sound outside the range
	 * 0.25 and 5.0).
	 *
	...
	/**
	 * Reflects the mediagroup HTML attribute, indicating the name of the group of elements it belongs to. A group of
	 * media elements shares a common controller.
	 *
	...
	/**
	 * Reflects the muted HTML attribute, indicating whether the media element's audio output should be muted by
	 * default. This property has no dynamic effect, to mute and unmute the audio output, use the muted property.
	 *
	 ...
	 * Note: Yet Gecko supports only single track playback, and the parsing of tracks' metadata is only available for
	 ...
	 * @param listener {Function} The object that receives a notification when an event of the specified type occurs.
	 *     This must be an object implementing the EventListener interface, or simply a JavaScript function.
	 * @param useCapture {boolean} If true, useCapture indicates that the user wishes to initiate capture. After
	 *     initiating capture, all events of the specified type will be dispatched to the registered listener before
	 *     being dispatched to any EventTarget beneath it in the DOM tree. Events which are bubbling upward through the
	 *     tree will not trigger a listener designated to use capture. See DOM Level 3 Events and JavaScript Event
	 *     order for a detailed explanation. If not specified, useCapture defaults to false.
	 */
	 ...
	 * @param useCapture {boolean} Specifies whether the EventListener to be removed was registered as a capturing
	 *     listener or not. If this parameter is absent, a default value of false is assumed. If a listener was
	 *     registered twice, one with capture and one without, each must be removed separately. Removal of a capturing
	 *     listener does not affect a non-capturing version of the same listener, and vice versa. Note: useCapture was
	 *     required in most major browsers' early versions. If broad compatibility is desired, you should always
	 *     provide the useCapture parameter.
	 */
	 ...
	 * Note: Previously canPlayType('video/webm') returned 'probably'. Starting with Gecko 28 (Firefox 28 / Thunderbird
	 * 28 / SeaMonkey 2.25 / Firefox OS 1.3), it returns 'maybe'.
	 *
	 * @method canPlayType
	...
	/**
	 * Reset the media element and restart selecting the media resource.  Any pending events are discarded.  How much
	 * media data is fetched is still affected by the preload attribute.  This method can be useful for releasing
	 * resources after any src attribute and source element descendants have been removed.  Otherwise, it is usually
	 * unnecessary to use this method, unless required to rescan source element children after dynamic changes.
	 *
	 * @method load
	 * @return void
	 */
