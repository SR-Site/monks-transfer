...
export default abstract class LoadTask<T> extends Disposable {
	...
	public load(update?: (progress: number) => void): Promise<void> {
		const batchProgress = this.batches.map(batch => 0);
	 ...
	 * @returns {Promise<any>}
	 */
	private loadBatch(item: IBatch, batchProgress: Array<number>, update: (progress: number) => void): Promise<any> {
...
export interface ILoadTaskOptions<T> {
	...
	onAssetLoaded?: (result: onAssetLoadedResponse<T>) => void;
}
